...
export interface AppState {
  ...
  // Recon state
  reconActiveTarget: string | null
  reconLogs: string[]
  reconResults: any | null
  ...
  // Recon actions
  setReconActiveTarget: (target: string | null) => void
  addReconLog: (log: string) => void
  setReconResults: (results: any | null) => void
  setIsReconLoading: (loading: boolean) => void
  clearRecon: () => void
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      // Recon initial state
      reconActiveTarget: null,
      reconLogs: [],
      reconResults: null,
      isReconLoading: false,

      // Recon actions
      setReconActiveTarget: (reconActiveTarget) => set({ reconActiveTarget }),
      addReconLog: (log) => set((state) => ({ reconLogs: [...state.reconLogs, log] })),
      setReconResults: (reconResults) => set({ reconResults }),
      setIsReconLoading: (isReconLoading) => set({ isReconLoading }),
      clearRecon: () => set({ reconActiveTarget: null, reconLogs: [], reconResults: null, isReconLoading: false }),

      // Restore from a full backup export
