...
from workers.tasks import run_recon_pipeline
...
async def manual_test(target_url):
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Create Scan Job with a default skill
        scan_job = ScanJob(
            target_id=target.id, 
            status=JobStatus.PENDING, 
            options={"skill": "subdomain_enumeration"}
        )
        db.add(scan_job)
        await db.commit()
    ...
    try:
        # Note: In a production setting, this would be queued via Celery
        await run_recon_pipeline(scan_id, target_url)
        ...
        async with AsyncSessionLocal() as db:
            ...
            if job.results_summary:
                try:
                    summary = json.loads(job.results_summary)
                    print(f"\n[AI REPORT]\n{summary.get('report', 'No report text')}")
                    print(f"\n[PDF PATH] {summary.get('pdf_path', 'N/A')}")
                except:
                    print(f"\n[RAW SUMMARY]\n{job.results_summary}")
            else:
